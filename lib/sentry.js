...
module.exports = function sentry (moduleOptions) {
  ...
  // Config warnings
  if (options.disabled) {
    logger.info('Sentry will not work because the disabled option is set')
  } else if (!options.dsn && !options.public_key) {
    logger.info('Sentry will not work because no DSN was found')
  }

  // Generate DSN
  // https://docs.sentry.io/quickstart/#about-the-dsn
  if ((!options.dsn || !options.dsn.length) && options.public_key && !options.disabled) {
  ...
  if (!options.disabled && options.config.dsn) {
    logger.info(options.config.dsn);
