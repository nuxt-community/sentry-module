...
export default function SentryModule (moduleOptions) {
  ...
  if (!options.disabled && !options.disableClientSide) {
    const apmIntegrations = {}
    const integrationsKeys = Object.keys(options.clientIntegrations)
    integrationsKeys.forEach((integrationName) => {
      const integration = options.clientIntegrations[integrationName]
      if (integration.apm) {
        apmIntegrations[integrationName] = integration
        delete options.clientIntegrations[integrationName]
      }
    })
