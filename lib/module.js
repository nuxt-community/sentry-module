...
export default async function SentryModule (moduleOptions) {
  ...
  // Try to determine "config.release" automatically from local repo at build time and use
  // that value at run time (since the project might no longer be within a repo).
  if (!options.config.release) {
    const SENTRY_OPTIONS_FILENAME = 'sentry.options.js'
    if (this.options._build) { // Naughty thing to use internal state but there is no better way...
      ...
      this.addTemplate({
        ...
        fileName: SENTRY_OPTIONS_FILENAME,
        options: {
      ...
      try {
        const optionsPath = resolve(this.options.buildDir, SENTRY_OPTIONS_FILENAME)
        const { config } = await import(optionsPath)
        options.config.release = config.release
      } catch {
        // Ignored
      }
    }
