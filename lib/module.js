...
export default function SentryModule (moduleOptions) {
  ...
  this.nuxt.hook('build:before', () => handleBuildHook(this, options))

  if (serverSentryEnabled(options)) {
    this.nuxt.hook('listen', () => handleListenHook(this, options))
  }

  // Enable publishing of sourcemaps
  if (options.publishRelease && !options.disabled && !this.options.dev) {
    this.nuxt.hook('webpack:config', webpackConfigs => handleWebpackConfigHook(this, webpackConfigs, options))
  }
}

export async function handleBuildHook (moduleContainer, options) {
...
export async function handleListenHook (moduleContainer, options) {
  await initSentry(moduleContainer, options)
}
...
export function handleWebpackConfigHook (moduleContainer, webpackConfigs, options) {
