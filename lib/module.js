...
export default async function SentryModule (moduleOptions) {
  ...
  const browserMethods = await getBrowserMethods()
  ...
  if (options.disabled || options.disableClientSide) {
    ...
    pluginOptionClient = 'async'
  } else {
    pluginOptionClient = 'client'
  }

  this.addPlugin({
    src: resolve(__dirname, `sentry.${pluginOptionClient}.js`),
  ...
  this.addPlugin({
    src: resolve(__dirname, `sentry.${pluginOptionServer}.js`),
...
async function getBrowserMethods () {
