...
export default function SentryModule (moduleOptions) {
  ...
  const privateRuntimeConfig = this.options.privateRuntimeConfig || {}
  const runtimeSentryConfig = (privateRuntimeConfig && privateRuntimeConfig.sentry) || {}
  const topLevelOptions = this.options.sentry || {}
  const options = deepMerge.all([defaults, topLevelOptions, moduleOptions, runtimeSentryConfig])
