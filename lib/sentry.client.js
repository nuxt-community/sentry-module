...
export default function (ctx, inject) {
  const opts = Object.assign({}, <%= serialize(options.config) %>, {
    integrations: [
      ...
      <%= Object.keys(options.apmIntegrations).map(name => {
        const integration = options.apmIntegrations[name]
        return `new ApmIntegrations.${name}({${Object.keys(integration).map(option => typeof integration[option] === 'function' ?
          `${option}:${serializeFunction(integration[option])}` : `${option}:${serialize(integration[option])}`).join(',')}})`
      }).join(',\n      ') %>
    ]
  })
