...
export async function resolveClientOptions (moduleContainer, moduleOptions, logger) {
  ...
  const customClientIntegrations = typeof (options.customClientIntegrations) === 'string' && options.customClientIntegrations
    ? moduleContainer.nuxt.resolver.resolveAlias(options.customClientIntegrations)
    : undefined
...
export async function resolveServerOptions (moduleContainer, moduleOptions, logger) {
  ...
  if (options.customServerIntegrations) {
    ...
    if (!Array.isArray(customIntegrations)) {
      console.error(`[@nuxtjs/sentry] Invalid value returned from customServerIntegrations plugin. Expected an array, got "${typeof (customIntegrations)}".`)
    }
  }
