...
export async function buildHook (moduleContainer, options, logger) {
  ...
  moduleContainer.addPlugin({
    ...
    options: {
      ...
      customClientIntegrationsPlugin: options.customClientIntegrationsPlugin,
...
export async function initializeServerSentry (moduleContainer, options) {
  ...
  if (canInitialize(options)) {
    ...
    if (options.customServerIntegrationsPlugin) {
      const resolvedPath = moduleContainer.nuxt.resolver.resolveAlias(options.customServerIntegrationsPlugin)
    ...
    Sentry.init({
      ...
      integrations: [
        // @ts-ignore
        ...filterDisabledIntegration(options.serverIntegrations).map(name => new Integrations[name](options.serverIntegrations[name])),
        ...customIntegrations
      ]
    })
  }
