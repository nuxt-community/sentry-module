...
export async function buildHook (moduleContainer, options, logger) {
  ...
  if (serverSentryEnabled(options)) {
    ...
    await initSentry(moduleContainer, options, logger)
  }
}
 ...
 * @return     {Promise<void>}
 */
export async function listenHook (moduleContainer, options, logger) {
  await initSentry(moduleContainer, options, logger)
}
...
export async function initSentry (moduleContainer, options, logger = console.info) {
