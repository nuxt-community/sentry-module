...
<% if (options.lazy.injectMock) { %>
let captureExceptionHandler = (err, vm, info) => {
  vm.$sentry.captureException(err)
}

const vueErrorHandler = VueLib.config.errorHandler

VueLib.config.errorHandler = (err, vm, info) => {
  captureExceptionHandler(err, vm, info)

  if (vueErrorHandler) {
    return vueErrorHandler(err, vm, info)
  }
}
<% } %>
...
// Use custom inject function that is able to overwrite previously injected values
