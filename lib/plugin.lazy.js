...
<%}
  ...
  const getCustomIntegrations = await import(/* <%= magicComments.join(', ') %> */ '<%= options.customClientIntegrations %>')
  ...
  if (Array.isArray(customIntegrations)) {
    ...
    console.warn(`[@nuxtjs/sentry] Invalid value returned from custom integrations function. Expected an array, got "${typeof customIntegrations}".`)
  }
  <% } %>
  /* eslint-enable object-curly-spacing, quote-props, quotes, key-spacing, comma-spacing */
  Sentry.init(config)
