...
export default async function SentryPlugin(ctx, inject) {
  ...
  tempInject(ctx, 'sentry', SentryMock)
  <% } %>
...
async function loadSentry(ctx, inject) {
  ...
  sentryReadyPromise = Promise.resolve(Sentry)
  loaded = true
...
function tempInject({ app }, key, value) {
  key = '$' + key
  // Add into app
  app[key] = value
  // Add into context
