...
function injectOverwrite({ app }, key, value) {
  ...
  let nuxt
  // window.$nuxt is set in App.create, which means
  // readyCallback always still needs to be called
  // when window.$nuxt has not yet been set
  if (window.<%= globals.nuxt %>) {
    setNuxtAppOption(window.<%= globals.nuxt %>, key, value)
  } else {
    window.<%= globals.readyCallback %>Cbs.push((app) => setNuxtAppOption(app, key, value))
  }
}

function setNuxtAppOption(nuxt, key, value) {
  nuxt.$options[key] = value
}
