  ...
  if (delayedGlobalErrors.length) {
    if (window.onerror) {
      ...
      for (const errorArgs of delayedGlobalErrors) {
        window.onerror.apply(window, errorArgs) // eslint-disable-line no-useless-call
      }
    }
    delayedGlobalErrors = []
  }
  if (delayedUnhandledRejections.length) {
    if (window.onunhandledrejection) {
      ...
      for (const reason of delayedUnhandledRejections) {
        window.onunhandledrejection.call(window, reason) // eslint-disable-line no-useless-call
      }
    }
